...
# The load test, and the thing being recorded.
#
# Compiling, linking and exporting the entry symbol are three things that can all be true
# while the class never registers. So this asserts the class exists before anything else and
# quits non zero if it does not.
#
...
func _ready() -> void:
	...
	# Typed explicitly. `:=` here infers Variant from ClassDB.instantiate, and this
	# project treats that warning as an error.
	var w: Object = ClassDB.instantiate("MovieWriterCineForm")
	if w == null:
		printerr("FAIL could not instantiate MovieWriterCineForm")
		get_tree().quit(1)
		return
	w.set_quality(2)
	w.set_thread_count(0)
	w.set_keep_alpha(false)
	print("ok   quality=%d threads=%d alpha=%s"
		% [w.get_quality(), w.get_thread_count(), str(w.get_keep_alpha())])
	# MovieWriter derives from Object, which is not reference counted, so this instance has to
	# be freed by hand. Leaving it leaks, and the leak is reported at exit as an anonymous
	# ObjectDB count that reads as if the extension were at fault.
	w.free()
